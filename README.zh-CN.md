...
- 127 点 XBAR V/F 读写
- MSVDD 补偿（+10mV），避免倒挂
- 验证组合：`XBAR +205 / MSVDD +10mV / ratio 1.2 / VF 自动宽范围 +88` → L2 PASS，日常稳妥档（更高测过 +215）
- 开源 / 可脚本化 / AI 可调用
...
- **使用说明**：[docs/USAGE.md](docs/USAGE.md) — RTX 50 用户如何使用向导。
- **技术说明**：[docs/TECHNICAL_NOTES.md](docs/TECHNICAL_NOTES.md) — 结论、NvAPI ID、版本、结构偏移，以及 **LACT #1147 / PR #1158 / issue #1159 / NVIDIA #1266** 的完整背景。
- **发现清单**：[docs/FINDINGS.md](docs/FINDINGS.md) — 所有发现（已用/未用）分门别类。
- **逆向笔记**：[docs/REVERSE_NOTES.md](docs/REVERSE_NOTES.md) — GET_INFO 完整解码。
- **驱动验证**：[docs/DRIVER_VALIDATION.md](docs/DRIVER_VALIDATION.md) — R572.16..R610.88 验证报告。
- **测试结果**：[TESTING.md](TESTING.md) — L2 稳定性记录。

## 硬件/驱动

支持的 RTX 50 系列（桌面+笔记本，同一套私有 NvAPI 布局）：

- 桌面：RTX 5050 / 5060 / 5060 Ti（8GB/16GB）/ 5070 / 5070 Ti / 5080 / 5090 / 5090 D / 5090 D v2
- 笔记本：RTX 5050 / 5060 / 5070 / 5070 Ti / 5080 / 5090 Laptop GPU（允许写入，但请谨慎）

运行时验证环境：

- NVIDIA RTX 5090（GB202）
- Windows 10/11 x64
- 静态验证通过的驱动：**572.16, 576.02, 580.88, 581.42, 591.86, 596.49, 610.62, 610.88**
