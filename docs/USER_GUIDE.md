...
1. 打开 mVolt+，确认电压/温度正常。
2. 运行 `python run.py status` 查看当前状态。
3. 运行 `python run.py wizard`，按提示输入：
   - 当前物理 MSVDD（从 mVolt+/HWiNFO 读取）
   - XBAR 偏移（建议从 +178 或 +205 开始）
   - MSVDD 偏移（+10 mV）
   - 比例（1.2）
   - VF 偏移（+88 MHz）
...
MSVDD offset : +10 mV
Ratio        : 1.2
VF offset    : +88 MHz (224..253 on RTX 5090)
```

- 这是作者机器上通过 L2 测试的日常稳妥档。
- 不同显卡/VBIOS/散热可能需要更低的值。
- 如果崩溃，先降到 +178 或 +150。
