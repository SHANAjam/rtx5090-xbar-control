...
| XBAR offset (MHz) | Extra XBAR clock offset | `205` = +205 MHz |
| MSVDD offset (mV) | Extra XBAR-domain voltage offset | `10` = +10 mV |
| Propagation ratio | GPC->XBAR ratio request | `1.2` |
| VF bank start/end | Which V/F points to change; the wizard auto-selects a broad range around your MSVDD | e.g. `224..253` |
...
MSVDD offset : 10 mV
Ratio        : 1.2
VF offset    : 88 MHz (224..253 on the author's RTX 5090)
```

This passed the L2 data-integrity test on the author's RTX 5090 and is the
current daily stable setting.
...
python run.py set-xbar --freq-khz 205000 --msvdd-uv 10000 --force-driver
